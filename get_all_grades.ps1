# Requirement: PowerShell (any version)
...
function Display-Content {
    ...
    for ($i = 0; $i -lt $content.Length; $i++) {
        if ($i -eq 0) {
            Write-Host $heading -ForegroundColor blue
        }
        ...
        Write-Host $courseCd -ForegroundColor green -NoNewLine
        Write-Host " | " -NoNewLine
        Write-Host $courseName -ForegroundColor magenta -NoNewLine
        Write-Host " | " -NoNewLine
        Write-Host $periodName -ForegroundColor cyan
...
function Get-Json {
    ...
    if ($statusCode -ne 200) {
        Write-Host "Something went wrong (HTTP status code $statusCode)."
        Write-Host "Exiting..."
        exit
    }
    ...
    try {
        ...
        Write-Host "Something went wrong. Could not convert the response content to JSON."
        Write-Host "Exiting..."
        exit
    }
}
